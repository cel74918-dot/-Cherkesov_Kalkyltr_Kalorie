...
Total = float(input("Введите сколько всего калорий можно потребить")) # В зависимости от диеты!
kall = 0
while Total > Final_calories:
    kall = Total - Final_calories
    print(f"Отлично для поддержания диеты вам необходимо набрать ещё {kall} ккал!")
    break
if Total < Final_calories:
    print(f"Срочно уберите лишние {Final_calories - Total} ккал, иначе требования не будут соответствовать диете!")
